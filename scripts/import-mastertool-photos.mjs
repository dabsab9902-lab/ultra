...
const CANDIDATES_FILE = join(ROOT, "reports", "mastertool-photo-candidates-top500.csv");
const SOURCES_FILE = join(ROOT, "scripts", "photo-map", "mastertool-photo-sources.json");
const PHOTO_MAP_FILE = join(ROOT, "scripts", "photo-map", "mastertool-photo-map.csv");
const REPORT_FILE = join(ROOT, "reports", "mastertool-photo-import-report.json");
const REPORT_CSV_FILE = join(ROOT, "reports", "mastertool-photo-import-report.csv");
const ARG_LIMIT = process.argv[2] || "";
const SCOPE = (
  process.env.MASTERTOOL_PHOTO_SCOPE ||
  (ARG_LIMIT.toLocaleLowerCase("uk") === "all" ? "all" : "csv")
).toLocaleLowerCase("uk");
const LIMIT =
  ARG_LIMIT.toLocaleLowerCase("uk") === "all"
    ? Number(process.env.MASTERTOOL_PHOTO_LIMIT || 0) || Infinity
    : Number(process.env.MASTERTOOL_PHOTO_LIMIT || ARG_LIMIT || 100);
const CONCURRENCY = Math.max(
  1,
  Math.min(4, Number(process.env.MASTERTOOL_PHOTO_CONCURRENCY || 3) || 3)
);
const USER_AGENT =
  "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/126 Safari/537.36";
const INSECURE_HTTPS_AGENT = new https.Agent({ rejectUnauthorized: false });

if (!existsSync(PRODUCTS_FILE)) throw new Error(`Missing ${PRODUCTS_FILE}`);
if (!existsSync(AGENT_PRODUCTS_FILE)) throw new Error(`Missing ${AGENT_PRODUCTS_FILE}`);
if (!existsSync(CANDIDATES_FILE)) throw new Error(`Missing ${CANDIDATES_FILE}`);
if (!existsSync(SOURCES_FILE)) throw new Error(`Missing ${SOURCES_FILE}`);
ensurePhotoMapFile();
...
const pwaProducts = Array.isArray(productsPayload.products)
  ? productsPayload.products
  : [];
const pwaById = new Map(
  pwaProducts.map((product, index) => [String(index + 1), product])
);
...
const allCandidates =
  SCOPE === "all"
    ? buildAgentPlusCandidates(agentRows)
    : readCsv(CANDIDATES_FILE).filter((row) => parseStock(row.stock) > 0);
...
async function processCandidate(candidate) {
  ...
  const brand = clean(candidate.brand) || "MASTERTOOL";
  ...
  const pwaProduct = agentRow.pwaProductId
    ? pwaById.get(String(agentRow.pwaProductId))
    : null;
  ...
  report.processed += 1;
  const result = await findPhoto({ article, name, brand });
  ...
  if (!result) {
    ...
    if (report.examplesSkipped.length < 12) report.examplesSkipped.push(item);
    return;
  }
  ...
  report.items.push(item);
  if (report.examplesFound.length < 12) report.examplesFound.push(item);
...
productsPayload.mastertoolPhotoImport = {
  generatedAt: report.generatedAt,
  limit: LIMIT,
  processed: report.processed,
  found: report.found,
  source: "configured product sources + manual CSV",
};
agentPayload.mastertoolPhotoImport = productsPayload.mastertoolPhotoImport;
...
async function findPhoto(product) {
  const manual = manualPhotoMap.get(normalizeArticle(product.article));
  ...
  for (const source of sourceConfig.sources) {
    if (source.enabled === false || source.id === "manual-map") continue;
    ...
    for (const pageUrl of pageUrls.slice(0, source.maxProductPages || 4)) {
      ...
      if (!scored.articleMatched) continue;
    ...
    await sleep(source.delayMs || 150);
  }
...
async function findProductUrlsInSource(source, product) {
  ...
  for (const searchUrl of source.searchUrls || []) {
    const url = searchUrl.replace("{article}", encodeURIComponent(article));
    const page = await fetchSearchPage(url, source);
    if (!page) continue;

    for (const link of extractArticleLinks(page.html, page.url, product, source)) {
      urls.push(link);
    }
  }
  ...
  for (const indexFile of source.urlIndexFiles || []) {
    ...
    for (const url of indexUrls) {
      if (articleAppears(`${url}`, article) && isAllowedHost(url, source)) {
        urls.push(url);
      }
    }
  }
  ...
  try {
    ...
    $('script[type="application/ld+json"]').each((_, element) => {
      try {
        ...
        // Broken JSON-LD is common in catalog pages.
      }
    });
    $("img").each((_, image) => {
      addImage(
...
function extractArticleLinks(html, baseUrl, product, source) {
  ...
  $("a").each((_, element) => {
    ...
    if (source.productPathHints?.length) {
      ...
      const hinted = source.productPathHints.some((hint) =>
        path.includes(String(hint).toLowerCase())
      );
...
function scorePage(product, page, source) {
  ...
  const brandText = compact(`${product.brand} ${page.title} ${page.text} ${page.url}`);
  const articleMatched =
    pageTitle.includes(article) || pageText.includes(article) || pageUrl.includes(article);
  const brandMatched =
    brandText.includes("mastertool") ||
    brandText.includes("mastertool") ||
    brandText.includes("master") ||
    compact(product.brand) && brandText.includes(compact(product.brand));

  let score = 0;
  if (articleMatched) score += 48;
  if (pageTitle.includes(article)) score += 18;
  if (pageUrl.includes(article)) score += 12;
  if (brandMatched) score += 8;
...
function isGoodImageUrl(url) {
  ...
  if (
    /(logo|sprite|icon|placeholder|no[-_]image|noimage|delivery|payment|youtube|favicon|banner|avatar|wishlist|compare|cart|flag)/i.test(
      lower
    )
  ) {
    return false;
  }
  if (/(\/design\/|\/brands?\/|\/categories?\/|\/publications?\/|\/media\/img\/)/i.test(lower)) {
    return false;
  }
...
function imageScore(imageUrl, pageUrl) {
  ...
  if (/\/(product|products|catalog|shop|files)\//i.test(lower)) score += 10;
  if (/cdn\.27\.ua|images\.prom\.ua|\/sc--media--prod\//i.test(lower)) score += 8;
...
function ensurePhotoMapFile() {
  ...
  if (!existsSync(PHOTO_MAP_FILE)) {
    writeFileSync(PHOTO_MAP_FILE, "article,imageUrl,sourceUrl\n", "utf-8");
  }
}
...
function hasValidImage(value) {
  ...
  return Boolean(
    raw &&
      raw !== "/product-fallback.svg" &&
      !raw.includes("product-fallback.svg") &&
      /^https?:\/\//i.test(raw)
  );
}
...
function record(status, candidate, reason) {
  ...
  report.items.push(item);
  if (status === "skipped" && report.examplesSkipped.length < 12) {
    report.examplesSkipped.push(item);
  }
}
...
function reportSummary(input) {
  return {
    ...
    alreadyHadPhotoBefore: input.alreadyHadPhotoBefore,
    fallbackBefore: input.fallbackBefore,
...
async function runCandidatePool(list, concurrency) {
  console.error(
    `[photos] start scope=${SCOPE} candidates=${list.length} eligible=${allCandidates.length} concurrency=${concurrency}`
  );
  let nextIndex = 0;
    ...
    while (nextIndex < list.length) {
      ...
      try {
        ...
        if (completedCandidates % 25 === 0 || completedCandidates === list.length) {
          console.error(
            `[photos] ${completedCandidates}/${list.length} done; found=${report.found}; skipped=${report.skipped}; notFound=${report.notFound}`
          );
        }
      }
    }
...
function buildAgentPlusCandidates(rows) {
  return rows
    .filter((row) => isMastertoolBranch(row) && parseStock(row.stock) > 0)
    .map((row, index) => ({
      rank: String(index + 1),
      score: "",
      article: firstString(row.article, row.sku, row.pwaSku, row.brandCode, extractArticle(row.name)),
      brand: firstString(row.brand, detectMastertoolBrand(row.name)),
      name: clean(row.name),
      ...
      has_article: firstString(row.article, row.sku, row.pwaSku, row.brandCode, extractArticle(row.name))
        ? "yes"
        : "no",
      has_brand: firstString(row.brand, detectMastertoolBrand(row.name)) ? "yes" : "no",
      has_name: clean(row.name) ? "yes" : "no",
    }));
}
...
function isMastertoolBranch(row) {
  return (row.categoryPath || []).some((part) => /master\s*tool|mastertool/i.test(clean(part)));
}

function detectMastertoolBrand(name) {
  const text = clean(name).toLocaleUpperCase("uk");
  ...
  if (text.includes("MASTERTOOL") || text.includes("MASTER TOOL")) return "MASTER TOOL";
  return "MASTER TOOL";
}
...
function extractArticle(name) {
  ...
  const alphaAtEnd = text.match(
    /(\b[A-ZА-ЯІЇЄҐ]{1,8}[- ]?\d{3,6}[A-ZА-ЯІЇЄҐ0-9-]*\b)\s*$/i
  );
  if (alphaAtEnd) return alphaAtEnd[1].replace(/\s+/g, "");

  const afterBrand = text.match(
    /(?:MASTER\s*TOOL|MASTERTOOL|ГОСПОДАР|GRANITE|WERTVOLL)\s*([A-ZА-ЯІЇЄҐ0-9][A-ZА-ЯІЇЄҐ0-9./-]{2,})\b/i
  );
  if (afterBrand) return afterBrand[1];

  const numericCodes = Array.from(
    text.matchAll(/\b\d{1,3}(?:[-/]\d{2,5}){1,3}[A-ZА-ЯІЇЄҐ0-9-]*\b/gi)
  ).map((match) => match[0]);
  if (numericCodes.length > 0) return numericCodes.at(-1) ?? "";

  const alphaCodes = Array.from(
    text.matchAll(/\b[A-ZА-ЯІЇЄҐ]{1,8}[- ]?\d{3,6}[A-ZА-ЯІЇЄҐ0-9-]*\b/gi)
  )
    .map((match) => match[0].replace(/\s+/g, ""))
    .filter((code) => !/^din\d+$/i.test(code));
