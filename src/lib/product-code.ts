...
const CODE_PATTERNS: Array<{ re: RegExp; score: number }> = [
...
export function isInternalSiteArticle(value: unknown) {
  return /^\d{2,8}$/.test(String(value ?? "").trim());
}

export function extractBrandCode({
  title = "",
  name = "",
  url = "",
  brand = "",
  ...
  const titleCandidates = collectCandidates(titleText, "title", brand, siteArticle);
...
export function resolveDisplayArticle({
  ...
  if (trustedBrandCode && !isNoArticleLabel(trustedBrandCode)) {
    if (isWeakCableCode(trustedBrandCode)) {
      weakCode ||= trustedBrandCode;
    } else {
      return trustedBrandCode;
    }
  }

  const extracted = extractBrandCode({ title, name, url, brand, siteArticle });
  if (weakCode) {
...
function collectCandidates(
  ...
  const brandIndex = brand
    ? sourceText.toLocaleLowerCase("ru").lastIndexOf(brand.toLocaleLowerCase("ru"))
    : -1;
  ...
  if (source === "title" && brandIndex >= 0) {
    const tail = sourceText.slice(brandIndex + brand.length);
    for (const match of tail.matchAll(/\b[A-Z]{1,6}\s+\d{1,6}[A-Z0-9/-]*(?:-[A-Z0-9]+)?\b/gi)) {
      ...
      const index = brandIndex + brand.length + (match.index ?? 0);
      candidates.push({
        code,
        index,
...
function urlToSearchText(url: string) {
  try {
...
function isBadCandidate(code: string) {
